...
function imagenet_acctest(model, ps, st, dev; size=224)
    ...
    top5 = TEST_LBLS[sortperm(ypred; rev=true)][1:5]
...
@testitem "EfficientNet" setup=[SharedTestSetup, PretrainedWeightsTestSetup] tags=[:vision] begin
for (mode, aType, dev, ongpu) in MODES, model_name in ["efficientnet-b0", "efficientnet-b1", "efficientnet-b2", "efficientnet-b3", "efficientnet-b4", "efficientnet-b5", "efficientnet-b6", "efficientnet-b7"]
    @testset for pretrained in [false, true]
        model = Boltz.Vision.EfficientNet(model_name; pretrained);
        ps, st = Lux.setup(Random.default_rng(), model) |> dev
        st = Lux.testmode(st)
        img = randn(Float32, 224, 224, 3, 2) |> aType
